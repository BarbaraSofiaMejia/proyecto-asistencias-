...
def aplicar_estilos():
    ...
    style.configure("TButton", font=("Arial", 11, "bold"), foreground="#004d61")
    style.map("TButton",
              foreground=[('active', '#0a9396')],
              background=[('active', '#94d2bd')])

def crear_entrada(frame, texto, fila):
    ttk.Label(frame, text=texto).grid(row=fila, column=0, pady=8, sticky="e", padx=10)
    entrada = ttk.Entry(frame, width=30)
    entrada.grid(row=fila, column=1, pady=8, padx=10)
    return entrada

def crear_combobox(frame, texto, fila, valores):
    ttk.Label(frame, text=texto).grid(row=fila, column=0, pady=8, sticky="e", padx=10)
    combobox = ttk.Combobox(frame, values=valores, state="readonly", width=28)
    combobox.grid(row=fila, column=1, pady=8, padx=10)
    combobox.current(0)
    return combobox
...
def registrar_empleado():
    ...
    ttk.Label(frame, text="Puesto:").grid(row=2, column=0, pady=8, sticky="e", padx=10)
    puesto_cb = ttk.Combobox(frame, values=puestos, state="readonly", width=28)
    puesto_cb.grid(row=2, column=1, pady=8, padx=10)
    puesto_cb.current(0)
...
def eliminar_empleado():
    ...
    def eliminar():
        ...
        with open(ARCHIVO, "r") as f:
            reader = csv.DictReader(f)
            for fila in reader:
                if not (fila["Nombre"].strip().lower() == nombre.get().strip().lower() and fila["Apellido"].strip().lower() == apellido.get().strip().lower()):
    ...
    ttk.Button(frame, text="Eliminar", command=eliminar).grid(row=2, column=0, columnspan=2, pady=15)
...
def registrar_asistencia():
    ...
    ttk.Button(frame, text="Registrar", command=registrar).grid(row=2, column=0, columnspan=2, pady=15)
...
def retardos_permisos():
    ...
    ttk.Button(frame, text="Guardar", command=guardar).grid(row=3, column=0, columnspan=2, pady=15)

def registrar_incapacidad():
    ventana = tk.Toplevel()
    ventana.title("Registrar Incapacidad")
    ventana.geometry("320x220")
    ventana.configure(bg="#c9e4de")
    aplicar_estilos()
    frame = ttk.Frame(ventana, padding="10")
    frame.pack()
...
def registrar_vacaciones():
    ...
    ttk.Button(frame, text="Guardar", command=guardar).grid(row=4, column=0, columnspan=2, pady=15)
...
def crear_menu():
    ...
    ventana.configure(bg="#81cfe0")

    tk.Label(ventana, text="Sistema de Control de Personal Hospitalario", font=("Arial", 16, "bold"), bg="#a9d6e5").pack(pady=30, fill=tk.X)

    def activar_menu():
        ventana.config(menu=menu_principal)
        boton_menu.pack_forget()
        tk.Label(ventana, text="Menú activado.", fg="green", bg="#a9d6e5", font=("Arial", 10)).pack()

    global boton_menu
    boton_menu = ttk.Button(ventana, text="Ir al menú de opciones", command=activar_menu)
    boton_menu.pack(pady=15)
