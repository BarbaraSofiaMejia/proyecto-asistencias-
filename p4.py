...
def crear_entrada(frame, texto, fila):
    ttk.Label(frame, text=texto, foreground="black").grid(row=fila, column=0, pady=5, sticky="e")
...
def registrar_empleado():
    ...
    ttk.Label(frame, text="Puesto:", foreground="black").grid(row=2, column=0, pady=5, sticky="e")
    ...
    entradas["Fecha Nacimiento"] = crear_entrada(frame, "Fecha de Nacimiento:", 5)
    ...
    entradas["Fecha Ingreso"] = crear_entrada(frame, "Fecha de Ingreso:", 12)
...
def registrar_festivo():
    ...
    fecha = crear_entrada(frame, "Fecha:", 1)
...
def registrar_permiso():
    ...
    inicio = crear_entrada(frame, "Fecha Inicio:", 3)
    fin = crear_entrada(frame, "Fecha Fin:", 4)

    def guardar():
        if empleado_existe(nombre.get(), apellido.get()):
            with open(ARCHIVO_PERMISOS, "a", newline="") as f:
                writer = csv.writer(f)
                writer.writerow([nombre.get(), apellido.get(), tipo.get(), inicio.get(), fin.get(), ""])
...
def registrar_vacaciones():
    ...
    inicio = crear_entrada(frame, "Fecha de inicio:", 3)
    fin = crear_entrada(frame, "Fecha de fin:", 4)
...
def registrar_asistencia():
    ...
    nombre = crear_entrada(frame, "Nombre:", 0)
    apellido = crear_entrada(frame, "Apellido:", 1)
    fecha = crear_entrada(frame, "Fecha:", 2)
    hora_entrada = crear_entrada(frame, "Hora de entrada:", 3)
    turno = crear_combobox(frame, "Turno:", 4, ["Matutino", "Vespertino", "Noche", "Mixto", "Personalizado"])
...
def crear_menu():
    ...
    tk.Label(ventana, text="Sistema de Control de Personal Hospitalario", font=("Helvetica", 16, "bold"), fg="black", bg="#b2ebf2").pack(pady=30)
